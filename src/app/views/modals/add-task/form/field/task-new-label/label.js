const Label = (title) => {
  ...
  container.classList.add('task-label');
  ...
  removeLabelBtn.classList.add('remove-label-btn');
  removeLabelBtn.setAttribute('type', 'button');
  removeLabelBtn.innerHTML = `<svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
                  viewBox="0 0 16 16"
  ...
  container.append(labelName, removeLabelBtn);

  removeLabelBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    container.remove();

    const labelItem = document.querySelector('.task-labels .task-label');
    if (!labelItem) {
      const taskLabelsSection = document.querySelector('.task-labels');
      taskLabelsSection.classList.add('hidden');
    }
  });
